...
mitigator_lookup <- AzureStor::storage_read_csv(
  ...
  dplyr::filter(is.na(active_to))  # non-NA active_to means it's deprecated
...
mitigator_values_df <- extract_params(
  ...
  dplyr::filter(!is.na(`Mitigator code`))  # discard mitigators not in lookup
```
...
The only scenarios shown are those that are labelled internally with a run stage of 'final_report_ndg2', i.e. the primary used in an outputs report.
